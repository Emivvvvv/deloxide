...
/**
 * Convert raw lock number to letter representation
 * e.g., 1 → "A", 2 → "B", 3 → "C", ...
 */
function mapLockId(lockNum) {
  const letters = "ABCDEFGHIJKLMNOPQRSTUVWXYZ";
  return letters[lockNum - 1] || lockNum.toString();
}
...
function transformLogs(rawLogs, resourceMapping) {
  ...
  const initLog = {
    ...
    type: "init",
    description: `Created threads ${rawThreadIds.join(", ")} and resources ${allLockLetters.map(r => r).join(", ")}.`
  };
  ...
  const normalLogs = rawLogs.map((log, idx) => {
    ...
    return {
      ...
      description: `<span class="thread-id">Thread ${rawThread}</span> ${eventTypes[eventCode] || "unknown"} <span class="resource-id">Resource ${resourceMapping[lockNum]}</span>`
    };
  });

  // Deadlock log: Collect "attempt" events from normal logs and create a generic description
  const unresolvedEvents = normalLogs.filter(log => log.type === "attempt");
  // Deadlock cycle: Contains raw thread ids that are waiting
  const deadlockCycle = Array.from(new Set(unresolvedEvents.map(ev => ev.thread_id))).sort();
  
  // Create a clear description of the deadlock situation
  const deadlockDescriptions = unresolvedEvents.map(ev =>
  ...
  if (deadlockDescriptions.length === 1) {
    ...
    deadlockDescription = `<strong>DEADLOCK DETECTED:</strong> ${deadlockDescriptions[0]} while ${deadlockDescriptions[1].replace('<span class="thread-id">Thread', '<span class="thread-id">thread')}`;
  } else {
    const lastDesc = deadlockDescriptions.pop();
    deadlockDescription = `<strong>DEADLOCK DETECTED:</strong> ${deadlockDescriptions.join(", ")} and ${lastDesc.replace('<span class="thread-id">Thread', '<span class="thread-id">thread')}`;
  }
  
  // Last log timestamp: 100 ms after the last event (in milliseconds)
  const lastTimestamp = rawLogs.length ? rawLogs[rawLogs.length - 1][3] : Date.now() / 1000;
...
function generateGraphStateFromLogs(logs, graphThreadMapping, resourceMapping) {
  ...
  const normalEvents = logs.filter(log => log.type !== "init" && log.type !== "deadlock");
  ...
  const deadlockThreads = logs.find(log => log.type === 'deadlock')?.cycle || [];
  ...
  if (deadlockThreads.length >= 2) {
    ...
    for (let i = 0; i < deadlockThreads.length; i++) {
      ...
      const nextThread = `T${graphThreadMapping[deadlockThreads[(i + 1) % deadlockThreads.length]]}`;
      deadlockLinks.push({ source: currentThread, target: nextThread, type: "deadlock" });
    }
  } else {
...
function transformRawObject(rawData) {
  ...
  const graph_state = generateGraphStateFromLogs(logs, graphThreadMapping, resourceMapping);
